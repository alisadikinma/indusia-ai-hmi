...
export const reviewOverrideSchema = z.object({
  action: z.enum(['approve', 'reject', 'review'], {
    errorMap: () => ({ message: 'Action must be approve, reject, or review' })
  }),
  reviewer_id: z.string().min(1, 'Reviewer ID is required'),
  reviewer_name: z.string().min(1, 'Reviewer name is required'),
  reviewer_notes: z.string().optional(),
  frame_decisions: z.record(z.string(), z.enum(['approved', 'rejected'])).optional()
}).refine(
  (data) => data.action !== 'review' || (data.frame_decisions && Object.keys(data.frame_decisions).length > 0),
  { message: 'frame_decisions is required when action is review', path: ['frame_decisions'] }
)
