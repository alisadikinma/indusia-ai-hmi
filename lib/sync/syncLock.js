...
export async function acquireLock(pcName = getPcName()) {
  ...
  if (!data || data.length === 0) {
    ...
    // If we didn't get lock but no one holds it, there's stale data
    // This can happen if sync crashed mid-way or lock row has inconsistent state
    if (!status.locked && !status.lockedBy) {
      console.log('[SyncLock] Detected inconsistent lock state, force clearing...')
