...
/**
 * Normalize role ID to simple name
 * Handles both 'role_operator' and 'operator' formats
 * @param {string} roleId - Role ID from database or simple name
 * @returns {string} Normalized role name
 */
function normalizeRole(roleId) {
  if (!roleId) return ''
  // Handle both 'role_operator' and 'operator' formats
  return roleId.replace(/^role_/, '')
}

/**
 * Check if a user role has a specific permission
 * @param {string} userRole - The user's role ID (e.g., 'role_operator' or 'operator')
...
export function hasPermission(userRole, permission) {
  ...
  // Normalize role ID (handle 'role_operator' -> 'operator')
  const normalizedRole = normalizeRole(userRole)
  const hasIt = allowedRoles.includes(normalizedRole)
  
  if (!hasIt) {
    console.warn(`[RBAC] Permission check failed: role=${userRole} (normalized=${normalizedRole}), permission=${permission}, allowed=${allowedRoles.join(',')}`)
  }
...
export function isRoleHigherOrEqual(roleA, roleB) {
  ...
  const levelA = ROLE_HIERARCHY[normalizedA] ?? -1
  const levelB = ROLE_HIERARCHY[normalizedB] ?? -1
...
export function canManageUser(managerRole, targetRole) {
  ...
  const managerLevel = ROLE_HIERARCHY[normalizedManager] ?? -1
  const targetLevel = ROLE_HIERARCHY[normalizedTarget] ?? -1
