...
/**
 * Get cloud image upload function (lazy load, server-side only)
 */
async function getUploadFunction() {
  if (typeof window !== 'undefined') {
    return null // Client-side, no upload
  }
  try {
    const mod = await import('@/lib/sync/cloudImageUpload')
    return mod.uploadOverrideImages
  } catch (err) {
    console.warn('[overridesRepo] Cloud image upload not available:', err.message)
    return null
  }
}
...
export async function approve(id, reviewerId, reviewerName, notes = '', uploadImages = true) {
  ...
  try {
    ...
    if (uploadImages && existing?.local_image_paths) {
      try {
        // Get upload function (lazy load)
        const uploadFn = await getUploadFunction()
        
        if (uploadFn) {
          console.log(`[overridesRepo.approve] Uploading images for override ${id}...`)
          console.log(`[overridesRepo.approve] local_image_paths:`, existing.local_image_paths)
          imageUploadResult = await uploadFn(id, existing.local_image_paths)
          ...
          if (imageUploadResult.success && imageUploadResult.cloudPaths) {
            ...
            console.warn('[overridesRepo.approve] Image upload returned:', imageUploadResult)
          }
        } else {
          console.log('[overridesRepo.approve] Upload function not available (client-side or not configured)')
        }
      } catch (imgErr) {
      ...
      console.log(`[overridesRepo.approve] No images to upload. uploadImages=${uploadImages}, local_image_paths=${existing?.local_image_paths}`)
    }
