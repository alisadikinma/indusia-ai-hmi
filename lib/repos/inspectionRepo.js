...
/**
 * Create inspection from AI Backend
 * @param {Object} data - AI inspection data
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function createFromAi(data) {
  try {
    const {
    ...
    const { data: inspection, error } = await supabase
      .from('inspection_results')
      .insert({
        external_inspection_id: externalInspectionId,
        line_id: lineId,
        ai_model_id: modelId,
        ai_model_version: modelName,
        ai_result: aiDecision,
        ai_timestamp: aiTimestamp,
        inspection_timestamp: aiTimestamp,
        ai_image_url_top: aiImageUrlTop,
        ai_image_url_bottom: aiImageUrlBottom,
        ai_objects_top: aiObjectsTop,
        ai_objects_bottom: aiObjectsBottom,
      })
      .select()
      .single();

    if (error) {
      console.error('[InspectionRepo] createFromAi error:', error);
      return { success: false, error: error.message };
    }
...
/**
 * List inspections for AI Backend with filters
 * @param {Object} filters - Query filters
 * @returns {Promise<{success: boolean, data?: Array, meta?: Object, error?: string}>}
 */
export async function listForAi(filters = {}) {
  try {
    const {
      lineId,
      aiDecision,
    ...
    if (lineId) query = query.eq('line_id', lineId);
    if (aiDecision) query = query.eq('ai_result', aiDecision);
    if (typeof isFalseCall === 'boolean') {
      if (isFalseCall) {
        query = query.eq('operator_decision', 'FALSE_CALL');
      } else {
        query = query.neq('operator_decision', 'FALSE_CALL');
      }
    }
    if (dateFrom) query = query.gte('inspection_timestamp', dateFrom);
    if (dateTo) query = query.lte('inspection_timestamp', dateTo);
    ...
    console.error('[InspectionRepo] listForAi error:', error);
    return { success: false, error: error.message };
  }
}

/**
 * Get inspection by external ID
 * @param {string} externalId - External inspection ID
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function getByExternalId(externalId) {
  try {
    const { data, error } = await supabase
      .from('inspection_results')
      .select('*')
      .eq('external_inspection_id', externalId)
      .single();
    ...
    console.error('[InspectionRepo] getByExternalId error:', error);
    return { success: false, error: error.message };
  }
}

/**
 * Update inspection by external ID
 * @param {string} externalId - External inspection ID
 * @param {Object} updates - Fields to update
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function updateByExternalId(externalId, updates) {
  try {
    const { data, error } = await supabase
      .from('inspection_results')
      .update(updates)
    ...
    console.error('[InspectionRepo] updateByExternalId error:', error);
    return { success: false, error: error.message };
  }
}
