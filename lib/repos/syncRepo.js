 ...
 * Uses sync_log table which is always populated after each sync
 */
export async function getLastSyncInfo() {
  ...
  if (error) {
    ...
    if (error.code !== 'PGRST116') {
      console.error('[SyncRepo] Error getting last sync info:', error)
    }
    return null
  }
  ...
  return data ? {
    ...
    tables: data.tables_synced,
    successCount: data.records_success,
    failedCount: data.records_failed,
