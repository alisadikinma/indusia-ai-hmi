...
export async function updateWorkOrderCounters(id, counters) {
  try {
    ...
    const newGoodQty = current.good_qty + (goodQty || 0);
    const newNgQty = current.ng_qty + (ngQty || 0);
    const newFalseCallQty = current.false_call_qty + (falseCallQty || 0);
    const newCompletedQty = current.completed_qty + (completedQty || 0);
    ...
    // if (newCompletedQty >= current.lot_size) {
