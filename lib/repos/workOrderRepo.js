...
/**
 * Check if a WO number is available (not already used)
 * @param {string} woNumber - The WO number to check
 * @returns {Promise<{success: boolean, available?: boolean, error?: string}>}
 */
async function checkWONumberAvailable(woNumber) {
  try {
    const { data, error } = await supabase
      .from('work_orders')
      .select('id')
    ...
    console.error('[WorkOrderRepo] Check WO number error:', error);
    return { success: false, error: error.message };
  }
}
...
export async function createWorkOrder(data) {
  try {
    const {
      ...
      notes,
      createdBy,
      woNumber: clientWoNumber,
    } = data;

    const woNumber = clientWoNumber?.trim() || await generateWONumber();

    // Check uniqueness before insert (friendly error vs raw DB constraint)
    const availCheck = await checkWONumberAvailable(woNumber);
    if (availCheck.success && !availCheck.available) {
      return { success: false, error: `Work order number "${woNumber}" already exists` };
    }
...
export async function getWorkOrders(filters = {}) {
  try {
    ...
    // Batch-resolve completed_by user IDs to names
    const completedByIds = [...new Set(
      data.filter(wo => wo.completed_by).map(wo => wo.completed_by)
    )];
    let userNameMap = {};
    ...
    return {
      success: true,
      data: data.map(wo => transformWorkOrder(wo, userNameMap)),
      count,
    };
  } catch (error) {
...
export async function startWorkOrder(id) {
  try {
    // Fetch current WO details for validation
    const { data: wo } = await supabase
      .from('work_orders')
      .select('line_id, board_id')
      .eq('id', id)
      .single();

    if (wo) {
      // Check if there's already an active WO on this line for the same board/model
      ...
      if (activeWO) {
        return {
          success: false,
          error: `Cannot start WO. ${activeWO.wo_number} is still active for this model. Please hold or complete it first.`,
        };
      }
    }
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function completeWorkOrder(id, { reason, completedBy } = {}) {
  try {
    const updateData = {
      status: 'completed',
      completed_at: new Date().toISOString(),
    };
    if (reason) updateData.completion_reason = reason;
 ...
 * @returns {Object}
 */
function transformWorkOrder(wo, userNameMap = {}) {
  if (!wo) return null;
  ...
  return {
    ...
    completionReason: wo.completion_reason,
    completedBy: wo.completed_by,
    completedByName: userNameMap[wo.completed_by] || wo.created_by_user?.name || null,
    notes: wo.notes,
    // Joined relations
...
export const workOrderRepo = {
  ...
  generateNextNumber: generateWONumber,
  checkNumberAvailable: checkWONumberAvailable,
};
