...
export async function startInspection(options = {}) {
  ...
  const endpoint = `${AI_BACKEND_URL}/api/inspection/start${qs ? `?${qs}` : ''}`
...
// ============================================
// PLC Simulation (Dummy Camera Mode)
// ============================================

/**
 * POST /serial/sim/in - Trigger simulated PLC READY signal
 * Required in dummy camera mode (AI_EDGE_DEBUG_CAMERA=true) to simulate
 * a board arriving on the conveyor. In production, the real PLC sends
 * this signal automatically. Silently fails if endpoint doesn't exist.
 * @returns {Promise<{success: boolean, error?: string}>}
 */
/**
 * POST /serial/sim/reset - Reset simulation serial files
 * Clears leftover PLC commands from previous sessions that cause rapid-fire loops.
 * @returns {Promise<{success: boolean, error?: string}>}
 */
export async function resetSimSerial() {
  const endpoint = `${AI_BACKEND_URL}/serial/sim/reset`
  ...
  try {
    ...
    console.log('[AI Backend] Simulation serial files reset')
    return { success: true }
  } catch (error) {
    console.warn('[AI Backend] resetSimSerial unavailable (OK in production):', error.message)
...
export async function triggerSimReady() {
  ...
  try {
    ...
    // Silently fail - endpoint only exists in simulation/dummy camera mode
