...
export async function runUpdatePipeline({ fromVersion, toVersion, onProgress, userId, skipRestart = false }) {
  ...
  try {
    ...
    try {
      // Reset package.json to match remote before pull.
      // The pipeline bumps package.json version locally after each update,
      // which would cause "git pull" to fail on subsequent updates.
      try {
        exec('git checkout -- package.json')
      } catch (_) { /* ignore — file may not be dirty */ }
