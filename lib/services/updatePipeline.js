...
export async function runUpdatePipeline({ fromVersion, toVersion, onProgress, userId }) {
  ...
  try {
    ...
    // Update package.json version to match target tag
    ...
    try {
      ...
      if (pkg.version !== targetSemver) {
        ...
        onProgress('GIT_PULL', 'running', `Updated package.json version to ${targetSemver}`)
      }
    } catch (err) {
      onProgress('GIT_PULL', 'running', `Warning: Could not update package.json version: ${err.message}`)
    }
    ...
    // Step 6: RESTART
    if (isDev) {
      onProgress('RESTART', 'running', 'Development mode — restart dev server manually (Ctrl+C → npm run dev)')
      onProgress('RESTART', 'done', 'Update complete. Restart dev server to apply changes.')
    } else {
