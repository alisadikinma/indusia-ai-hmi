# INDUSIA AI Database Schema v2

**Updated:** 2024-12-01
**Changes from v1:**
- Vector dimension: 1536 → 384 (gte-small)
- Index type: ivfflat → hnsw (3-6x faster)
- Distance operator: `<->` → `<=>` (cosine)
- Added: 6 new tables for training pipeline
...
CREATE INDEX idx_override_images_override ON override_images(override_id);
CREATE INDEX idx_override_images_batch ON override_images(training_batch_id) WHERE training_batch_id IS NOT NULL;
CREATE INDEX idx_override_images_unprocessed ON override_images(is_processed) WHERE is_processed = FALSE;
```

---

## 🧠 9. SEMANTIC SEARCH (UPDATED)
...
CREATE INDEX idx_dataset_images_source ON dataset_images(source_type, source_id);
CREATE INDEX idx_dataset_images_unannotated ON dataset_images(dataset_id, is_annotated) WHERE is_annotated = FALSE;
```

---

## 🚀 12. TRAINING JOBS *(NEW)*
...
CREATE INDEX idx_training_jobs_dataset ON training_jobs(dataset_id);
CREATE INDEX idx_training_jobs_created ON training_jobs(created_at DESC);
```

---

## 📊 13. TRAINING METRICS *(NEW)*
...
CREATE INDEX idx_training_metrics_epoch ON training_metrics(job_id, epoch);
```
...
CREATE INDEX idx_ai_models_created ON ai_models(created_at DESC);
```

---

## 🧮 15. RPC FUNCTIONS (UPDATED)

### 15.1 match_overrides
```sql
...
VALUES ('model-weights', 'model-weights', true, 524288000);
...
| # | Table | New/Updated | Purpose |
|---|-------|-------------|---------|
| 1 | customers | - | Master data |
| 2 | sections | - | Master data |
| 3 | customer_sections | - | Mapping |
| 4 | lines | - | Master data |
| 5 | boards | - | Master data |
| 6 | roles | - | RBAC |
| 7 | menu_items | - | RBAC |
| 8 | role_menu_permissions | - | RBAC |
| 9 | users | - | User accounts |
| 10 | notifications | - | Notifications |
| 11 | event_log | - | Audit log |
| 12 | system_events | - | System health |
| 13 | overrides | - | False call overrides |
| 14 | **override_images** | ✅ NEW | Override attachments |
| 15 | override_embeddings | ✅ UPDATED | Vector search (384 dim) |
| 16 | kb_articles | - | Knowledge base |
| 17 | kb_embeddings | ✅ UPDATED | Vector search (384 dim) |
| 18 | **training_datasets** | ✅ NEW | Dataset batches |
| 19 | **dataset_images** | ✅ NEW | Dataset images + annotations |
| 20 | **training_jobs** | ✅ NEW | Modal.com job tracking |
| 21 | **training_metrics** | ✅ NEW | Per-epoch metrics |
| 22 | **ai_models** | ✅ NEW | Trained model versions |
...
If upgrading from v1, run these steps:

```sql
-- Step 1: Drop old indexes
DROP INDEX IF EXISTS override_embeddings_idx;
DROP INDEX IF EXISTS kb_embeddings_idx;

-- Step 2: Alter vector dimensions (requires empty tables or truncate)
ALTER TABLE override_embeddings ALTER COLUMN embedding TYPE VECTOR(384);
ALTER TABLE kb_embeddings ALTER COLUMN embedding TYPE VECTOR(384);

-- Step 3: Create new HNSW indexes
CREATE INDEX override_embeddings_idx ON override_embeddings
...
DROP FUNCTION IF EXISTS match_overrides(VECTOR, FLOAT, INT, TEXT, TEXT);
DROP FUNCTION IF EXISTS match_kb_articles(VECTOR, FLOAT, INT, TEXT, TEXT, TEXT);
-- Then run CREATE FUNCTION statements from section 15

-- Step 5: Create new tables (sections 8, 10-14)
...
Tables 10-14 dan 18-22 (training pipeline) digunakan oleh **Training Platform terpisah**.
HMI hanya membaca dari `ai_models` untuk deployment.
